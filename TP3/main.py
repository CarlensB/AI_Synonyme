...
clustering = subparser.add_parser('c', )
clustering.add_argument('-t', type=int, required=True)
clustering.add_argument('-n', type=int, required=True)
clustering.add_argument('-k', type=int, required=True)
        ...
        with Dao() as dao:
            if opts.options == 'b':
                ...
                if opts.options == 'e':
                    ...
                    e.tfen = opts.t
                    e.construire_matrice()
                    c = KMeans(opts.t, opts.k, e.matrice, opts.n, e.vocabulaire)
    except EOFError:
        print_exc()
        return 1
    return 0
